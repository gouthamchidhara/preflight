...
function ps(script: string, params: unknown, mocks = ''): { json: Record<string, unknown>; raw: string } {
  const dir = tmp();
  const mockFile = join(dir, 'mocks.ps1');
  writeFileSync(mockFile, mocks);
  const r = spawnSync(PWSH, ['-NoProfile', '-NonInteractive', '-Command', `. '${mockFile}'; & '${join(CHECKS, script)}'`], {
  ...
  return { json: JSON.parse(line), raw };
}
...
describe.skipIf(!hasPwsh)('PowerShell check scripts', () => {
  it('every .ps1 parses (Windows PowerShell syntax subset)', () => {
  ...
  it('every check script emits exactly one valid JSON verdict, even when Windows cmdlets are missing', () => {
  ...
  describe('dataDisk (4 TB drive)', () => {
    it('present: one internal 4 TB disk passes', () => {
      const { json } = ps('dataDisk.ps1', dd('present'), diskMocks([OS, DATA]));
      expect(json.status).toBe('pass');
      expect(String(json.actual)).toContain('4.00 TB');
    });
    it('present: nothing detected fails with the disk list as evidence', () => {
      const { json } = ps('dataDisk.ps1', dd('present'), diskMocks([OS]));
      expect(json).toMatchObject({ status: 'fail', actual: '0 disks found' });
      expect(JSON.stringify(json.evidence)).toContain('OS SSD');
    });
    it('present: a 4 TB USB disk does not count', () => {
      expect(ps('dataDisk.ps1', dd('present'), diskMocks([OS, { ...DATA, bus: 'USB' }])).json.status).toBe('fail');
    });
    it('present: two candidates fails', () => {
      expect(ps('dataDisk.ps1', dd('present'), diskMocks([OS, DATA, { ...DATA, n: 2 }])).json.actual).toBe('2 candidate disks');
    });
    it('unconfigured: RAW passes, partitioned fails', () => {
      expect(ps('dataDisk.ps1', dd('unconfigured'), diskMocks([OS, DATA])).json.status).toBe('pass');
      const { json } = ps('dataDisk.ps1', dd('unconfigured'), diskMocks([OS, { ...DATA, style: 'GPT' }], 'PARTS'));
      expect(json.status).toBe('fail');
      expect(String(json.actual)).toContain('GPT, 1 partition(s) E:');
    });
    it('unconfigured: skipped (precondition) when the drive is missing', () => {
      expect(ps('dataDisk.ps1', dd('unconfigured'), diskMocks([OS])).json).toMatchObject({ status: 'skip', skipReason: 'precondition' });
    });
    it('health: unhealthy fails', () => {
      expect(ps('dataDisk.ps1', dd('health'), diskMocks([OS, { ...DATA, health: 'Warning' }])).json.status).toBe('fail');
      expect(ps('dataDisk.ps1', dd('health'), diskMocks([OS, DATA])).json.status).toBe('pass');
    });
    it('seated: I/O error events on that disk fail', () => {
      expect(ps('dataDisk.ps1', dd('seated'), diskMocks([OS, DATA])).json.status).toBe('pass');
      const { json } = ps('dataDisk.ps1', dd('seated'), diskMocks([OS, DATA], 'EVENTS'));
      expect(json.status).toBe('fail');
      expect(String(json.actual)).toContain('I/O error');
    });
    it('oldRemoved: leftover 256 GB disk fails', () => {
      const { json } = ps('dataDisk.ps1', dd('oldRemoved'), diskMocks([OS, DATA, { n: 2, name: 'Old SSD', size: 256e9 }]));
      expect(json.status).toBe('fail');
  ...
  describe('cellular', () => {
    ...
    it('mbnReady: Initialized passes, SIM not inserted fails', () => {
      expect(ps('mbnReady.ps1', {}, netsh('"Ready info for interface Cellular:`n    Ready State : Initialized"')).json.status).toBe('pass');
      const { json } = ps('mbnReady.ps1', {}, netsh('"    Ready State : SIM not inserted"'));
      expect(json).toMatchObject({ status: 'fail', actual: 'SIM not inserted' });
    });
    it('apnConfig: finds APN in profile XML', () => {
      const mocks = netsh(`
      ...
      const { json } = ps('apnConfig.ps1', { apn: { profileName: 'AA FirstNet', apn: '32871.fn' } }, mocks);
      expect(json.status).toBe('pass');
      const wrong = ps('apnConfig.ps1', { apn: { profileName: 'AA FirstNet', apn: 'other.apn' } }, mocks).json;
      expect(wrong.status).toBe('fail');
    });
  });

  describe('files', () => {
    it('fileSetPattern: complete Toolbox set passes; missing part and old stamp fail', () => {
      const dir = tmp();
      ...
      expect(ps('fileSetPattern.ps1', { dir, minStamp: stamp, parts: 6 }).json).toMatchObject({ status: 'pass', actual: `${stamp}, 6/6 parts` });
      expect(ps('fileSetPattern.ps1', { dir, minStamp: '20270101000000', parts: 6 }).json.status).toBe('fail');
      const dir2 = tmp();
      mkdirSync(join(dir2, 'deploy'));
      for (const n of names.filter((n) => !n.includes('Part_4'))) writeFileSync(join(dir2, n), 'x');
      const { json } = ps('fileSetPattern.ps1', { dir: dir2, minStamp: stamp, parts: 6 });
      expect(json.status).toBe('fail');
      expect(String(json.actual)).toContain('Part_4');
    });

    it('fileHash: compares to source when golden is TBD, to golden when set', () => {
      const dir = tmp();
      ...
      expect(ps('fileHash.ps1', { ...base, sha256: 'TBD(P0-9)' }).json.status).toBe('pass');
      expect(ps('fileHash.ps1', { ...base, sha256: golden }).json.status).toBe('pass');
      writeFileSync(join(dir, 'dst.jks'), 'tampered');
      expect(ps('fileHash.ps1', { ...base, sha256: golden }).json.status).toBe('fail');
      expect(ps('fileHash.ps1', { ...base, path: join(dir, 'missing.jks'), sha256: golden }).json).toMatchObject({ status: 'fail', actual: 'file missing' });
    });

    it('propsKeyHash: never prints secret values', () => {
      const dir = tmp();
      ...
      const tbd = ps('propsKeyHash.ps1', { path: file, keyHashes: { deviceLoginPassword: 'TBD(P0-9)', trustStorePassword: 'TBD(P0-9)' } });
      ...
      const ok = ps('propsKeyHash.ps1', { path: file, keyHashes: { deviceLoginPassword: good } });
      expect(ok.json.status).toBe('pass');
      const bad = ps('propsKeyHash.ps1', { path: file, keyHashes: { deviceLoginPassword: 'sha256:00' } });
  ...
  describe('system', () => {
    it('osVersion: build compare and TBD → needs_human', () => {
      ...
      expect(ps('osVersion.ps1', { build: '26100' }, m).json).toMatchObject({ status: 'pass', actual: '26100.4652' });
      expect(ps('osVersion.ps1', { build: '22631' }, m).json.status).toBe('fail');
      expect(ps('osVersion.ps1', { build: 'TBD' }, m).json.status).toBe('needs_human');
      expect(ps('osVersion.ps1', { minUbr: 5000 }, m).json.status).toBe('fail');
    });
    it('identity: asset tag pattern', () => {
      ...
      expect(ps('identity.ps1', { assetTagPattern: '^QJ0681\\d{4}$' }, m('QJ06811378')).json.status).toBe('pass');
      expect(ps('identity.ps1', { assetTagPattern: '^QJ0681\\d{4}$' }, m(' ')).json).toMatchObject({ status: 'fail', actual: '(empty)' });
    });
    it('biosVersion: variant unknown accepts any current release', () => {
      ...
      expect(ps('biosVersion.ps1', { versions: { touch: 'TBD', nontouch: 'TBD' }, touchSkuPattern: 'TBD' }, m).json.status).toBe('needs_human');
      expect(ps('biosVersion.ps1', { versions: { touch: 'V3.00L14', nontouch: 'V2.00L20' }, touchSkuPattern: 'TBD' }, m).json.status).toBe('pass');
      expect(ps('biosVersion.ps1', { versions: { touch: 'V3.00L15', nontouch: 'V2.00L20' }, touchSkuPattern: 'TBD' }, m).json.status).toBe('fail');
      expect(ps('biosVersion.ps1', { versions: { touch: 'V3.00L14', nontouch: 'V2.00L20' }, touchSkuPattern: 'RZ$' }, m).json.status).toBe('pass');
    });
    it('pnpAbsent: Bluetooth radio present fails', () => {
      ...
      expect(ps('pnpAbsent.ps1', { class: 'Bluetooth' }, m('Bluetooth')).json.status).toBe('fail');
      expect(ps('pnpAbsent.ps1', { class: 'Bluetooth' }, m('Net')).json.status).toBe('pass');
    });
  });
});
